...
import game.world.base.command.ChangeMapCommand;
...
public class WorldService implements IWorldService {
    ...
    @Override
    public void changeMap(Player player, int mapId, boolean clientRequest) {
        ExecutorUtils.submit(player, ChangeMapCommand.valueOf(player, mapId, clientRequest));
    }
    ...
    public void gatewayChangeMap(Player player, int mapId, boolean clientRequest) {
        ...
        try {
            ...
            // 允许玩家从无地图状态切过来
            if (oldMapId > 0) {
                gatewayLeaveMap(player, oldMapId, clientRequest);
            }
